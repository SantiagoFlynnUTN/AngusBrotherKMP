...
import com.angus.api_gateway.data.model.restaurant.RestaurantDto
import com.angus.api_gateway.data.model.restaurant.getRestaurantOptions
import com.angus.api_gateway.data.service.IdentityService
import com.angus.api_gateway.data.service.ImageService
        ...
        authenticateWithRole(Role.DASHBOARD_ADMIN) {
            post {
                ...
                identityService.updateUserPermission(
                    userId = user.id,
                    permission = listOf(user.permission, Role.RESTAURANT_OWNER),
                    language,
                )
        ...
        authenticateWithRole(Role.RESTAURANT_OWNER) {
            put("/details") {
                ...
                val updatedRestaurant = restaurantService.updateRestaurant(
                    languageCode = language,
                    isAdmin = false,
                    restaurantDto = newRestaurantDto,
                )
                respondWithResult(HttpStatusCode.OK, updatedRestaurant)
            }
        }
